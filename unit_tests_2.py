...
def test_k_multiple(state=None,WRITE=False, directory ='./Run/' ):
    ...
    these_ffts  = p49_eigen.get_ffts(state.cubes, means={})
    return state, these_ffts
#state, mask=test_k_array_powers(directory = './Run3', WRITE=True)
#ic_stuff=unit_tests.test_read_ics(ic_dir="./Run3",plot_dir="./Vis2/")
for frame in [0,1]:
    read_stuff=unit_tests.test_read_outputs(data_dir="./Run3",plot_dir="./Vis2/",frame=frame)
